...
(function() {
    ...
    function proxy(name) {
        ...
        if (name === 'filmix_site')
            return proxy_secret_ip || user_proxy1;
        if (name === 'filmix_abuse')
            return window.location.protocol === 'https:' ? 'https://cors.apn.monster/' : 'http://cors.cfhttp.top/';
        if (name === 'zetflix')
            return proxy_apn;
        if (name === 'allohacdn')
            return proxy_other ? proxy_secret : proxy_apn;
        if (name === 'cookie')
            return user_proxy1;
        if (name === 'cookie2')
            return user_proxy2;
        ...
        if (Lampa.Storage.field('online_mod_proxy_' + name) === true) {
            ...
            if (name === 'fancdn')
                return user_proxy3;
            if (name === 'fancdn2')
                return proxy_secret || user_proxy3;
            if (name === 'fanserials')
                return user_proxy2;
            if (name === 'videoseed')
                return user_proxy1;
            ...
            getStream(element, function(element) {
                ...
                if (element.season) {
                    ...
                    if (defaultPlayer === 'builtin') {
                        ...
                        // Для внешнего плеера - загружаем все ссылки сразу
    ...
    // Вспомогательная функция для запуска воспроизведения
    function startPlayback(first, playlist) {
        if (playlist.length > 1) {
            first.playlist = playlist;
        }
        Lampa.Player.play(first);
            ...
            network["native"](component.proxyLink(url, prox, prox_enc_page), function(str) {
                ...
                if (MOVIE_ID && PLAYER_CUID && IDENTIFIER && MOVIE_ID_SCRIPT && MOVIE_URL) {
                    ...
                    network["native"](component.proxyLink(movie_url, prox, prox_enc_page), function(script) {
                        ...
                        try {
                            params = (0,
                            eval)(decrypt + [JSON.stringify(component.decodeHtml(SCRIPTS[0])), JSON.stringify(component.decodeHtml(SCRIPTS[1])), JSON.stringify(script || ''), JSON.stringify('new'), JSON.stringify(file_type)].join(',') + ');');
                        } catch (e) {}
                        ...
                        network["native"](component.proxyLink(user_url, prox, prox_enc_page), function(data) {
                            if (data && !data.error) {
                                ...
                                try {
                                    _params = (0,
                                    eval)(decrypt + [JSON.stringify(component.decodeHtml(SCRIPTS[0])), JSON.stringify(component.decodeHtml(SCRIPTS[1])), JSON.stringify(script), JSON.stringify('new'), JSON.stringify(file_type), JSON.stringify(data)].join(',') + ');');
                                } catch (e) {}
                                ...
                                network["native"](component.proxyLink(vod_url, prox, prox_enc_page), function(files) {
                                    ...
                                    try {
                                        params = (0,
                                        eval)(decrypt + [JSON.stringify(component.decodeHtml(SCRIPTS[0])), JSON.stringify(component.decodeHtml(SCRIPTS[1])), JSON.stringify(script), JSON.stringify('new'), JSON.stringify(file_type), JSON.stringify(data), JSON.stringify(files)].join(',') + ');');
                                    } catch (e) {}
    ...
    function collaps(component, _object, prefer_dash) {
        ...
        var select_title = '';
        var prefer_http = Lampa.Storage.field('online_mod_prefer_http') === true;
        //let prefer_dash  = Lampa.Storage.field('online_mod_prefer_dash') === true
                ...
                item.on('hover:enter', function(event, options) {
                    ...
                    if (element.file) {
                        ...
                        if (options && options.runas)
                            Lampa.Player.runas(options.runas);
                        else if (lampa_player)
    ...
    function cdnmovies(component, _object) {
       ...
       */
        ...
        function parse(str, empty) {
            ...
            try {
                json = find && (0,
                eval)('"use strict"; (function(){ return ' + find[1] + '; })();');
            } catch (e) {}
        ...
        function find(filmix_id, abuse, abuse_error, low_quality) {
            ...
            function end_search() {
                ...
                var not_found = function not_found(str) {
                    if (abuse && abuse_error)
                        success(abuse_error);
                    else if (!abuse && abuse_token)
                        find(filmix_id, true, null, true);
                    else if (str)
                        component.empty(str);
                    else
                        component.emptyForQuery(select_title);
    ...
    function zetflix(component, _object) {
       ...
       */
        ...
        function getEpisodes() {
            ...
            if (s)
                url = Lampa.Utils.addUrlComponent(url, 's=' + s);
       ...
       */
        ...
        function parse(str) {
            ...
            try {
                json = find && (0,
                eval)('"use strict"; (function(){ var sign = function(){}; return ' + find[1] + '; })();');
            } catch (e) {}
    ...
    function fancdn(component, _object) {
        ...
        if (cookie.indexOf('PHPSESSID=') == -1)
            cookie = 'PHPSESSID=' + Utils.randomHex(32) + (cookie ? '; ' + cookie : '');
        ...
        this.search = function(_object, kinopoisk_id, data) {
            ...
            var display = function display(links) {
                if (links && links.length && links.forEach) {
                    ...
                    component.empty(Lampa.Lang.translate('online_mod_authorization_required') + ' FanSerials');
                else
                    component.emptyForQuery(select_title);
            };
        ...
        function fancdn_api_search(api, callback, error) {
            ...
            network["native"](component.proxyLink(url, prox, prox_enc), function(str) {
                if (str && str.indexOf('<title>Доступ ограничен</title>') !== -1) {
                    if (error)
                        error(Lampa.Lang.translate('online_mod_authorization_required') + ' FanSerials');
                    return;
                }
            ...
            try {
                var items = component.parseM3U(str).filter(function(item) {
                    ...
                    if (quality > 1440 && quality <= 2160)
                        quality = 2160;
                    else if (quality > 1080 && quality <= 1440)
                        quality = 1440;
       ...
       */
        ...
        function parse(str, empty) {
            ...
            if (find) {
                try {
                    json = find && (0,
                    eval)('"use strict"; (function(){ var token = "", domain_name = ""; return ' + find[1] + '; })();');
                } catch (e) {}
            } else {
    ...
    function alloha(component, _object) {
       ...
       */
        ...
        this.search = function(_object, kinopoisk_id) {
            ...
            alloha_api_search(api, function(json) {
                if (json && json.data && json.data.iframe)
                    getPage(json.data);
                else if (!object.clarification && object.movie.imdb_id && kinopoisk_id != object.movie.imdb_id) {
       ...
       */
        ...
        function parse(str, url) {
            ...
            try {
                extract = (0,
                eval)(decrypt + [JSON.stringify(str), JSON.stringify(url), JSON.stringify(token), JSON.stringify(av1_support)].join(',') + ');');
            } catch (e) {}
       ...
       */
        ...
        function getStream(element, call, error) {
            if (element.stream)
                return call(element);
            if (!(element.media.id && extract.domain))
                return error();
        ...
        function parse(json) {
            ...
            if (json && json.items && json.items.forEach) {
                ...
                items.sort(function(a, b) {
                    ...
                    if (cmp)
                        return cmp;
                    if ((a.voiceStudio || a.voiceType || '') > (b.voiceStudio || b.voiceType || ''))
                        return 1;
                    if ((a.voiceStudio || a.voiceType || '') < (b.voiceStudio || b.voiceType || ''))
                        return -1;
    ...
    function kodik(component, _object) {
            ...
            var display = function display(results, empty) {
                if (results && results.length && results.forEach) {
                    ...
                    results.forEach(function(c) {
                        var id;
                        if (c.shikimori_id)
                            id = 'sm_' + c.shikimori_id;
                        else if (c.worldart_link)
                            id = 'wa_' + c.worldart_link;
                        else if (c.kinopoisk_id)
                            id = 'kp_' + c.kinopoisk_id;
                        else if (c.imdb_id)
                            id = 'im_' + c.imdb_id;
                        else if (c.id)
                            id = 'k_' + c.id;
       ...
       */
        ...
        function success(json) {
            ...
            if (media.shikimori_id)
                params = Lampa.Utils.addUrlComponent(params, 'shikimori_id=' + encodeURIComponent(media.shikimori_id));
            else if (media.worldart_link)
                params = Lampa.Utils.addUrlComponent(params, 'worldart_link=' + encodeURIComponent(media.worldart_link));
            else if (media.kinopoisk_id)
                params = Lampa.Utils.addUrlComponent(params, 'kinopoisk_id=' + encodeURIComponent(media.kinopoisk_id));
            else if (media.imdb_id)
                params = Lampa.Utils.addUrlComponent(params, 'imdb_id=' + encodeURIComponent(media.imdb_id));
            else if (media.id)
                params = Lampa.Utils.addUrlComponent(params, 'id=' + encodeURIComponent(media.id));
            else {
                component.emptyForQuery(select_title);
                return;
            }
        ...
        function filter() {
            ...
            if (extract.seasons.length) {
                ...
                extract.items.forEach(function(c) {
                    if (!(c.seasons && c.seasons[season_id]))
                        return;
       ...
       */
        ...
        function decode(str) {
            try {
                if (startsWith(str, 'http') || startsWith(str, '//'))
                    return str;
    ...
    function kinopub(component, _object) {
       ...
       */
        ...
        function getVoiceName(audio, full) {
            ...
            if (voice_name)
                voice_name += audio.lang && audio.lang !== 'und' && audio.lang !== 'rus' ? ' (' + audio.lang + ')' : '';
            else
                voice_name = audio.lang || '';
            return voice_name;
        }
       ...
       */
        ...
        this.uniqueNamesShortText = function(names, limit) {
            ...
            if (limit && unique.length > 1) {
                ...
                unique.forEach(function(name, index) {
                    length += name.length;
                    if (limit_index == -1 && length > limit - (index == last_index ? 0 : 5))
                        limit_index = index;
                    length += 2;
                });
        ...
        this.find = function() {
            ...
            var display = function display(items) {
                if (items && items.length && items.forEach) {
                    ...
                    var is_imdb = false;
                    items.forEach(function(c) {
                        if (c.start_date === '1969-12-31')
                            c.start_date = '';
                        if (c.year === '1969-12-31')
                            c.year = '';
                ...
                if (!object.clarification && +object.movie.kinopoisk_id && kp_sources.indexOf(balanser) >= 0) {
                    ...
                    if (balanser == 'lumex' || balanser == 'lumex2') {
                        var fallback = function fallback() {
                            ...
                            } else if (Lampa.Storage.field('online_mod_skip_kp_search') !== true)
                                kp_search();
                            else
                                display([]);
                        };
                            ...
                            if (item.charAt(label_end + 1) === '{') {
                                item.substring(label_end + 2).split(/; *\{/).forEach(function(voice_item) {
                                    ...
                                    if (endsWith(voice_item, ';'))
                                        voice_item = voice_item.substring(0, voice_item.length - 1).trim();
        ...
        this.formatEpisodeTitle = function(s_num, e_num, name) {
            ...
            if (name == null || name === '')
                ...
                name = Lampa.Lang.translate('torrent_serial_episode') + ' ' + e_num + ' - ' + name;
            title += name;
            return title;
        }
        ;
       ...
       */
        ...
        this.similars = function(json, search_more, more_params) {
            ...
            json.forEach(function(elem) {
                ...
                var info = [];
                if (orig_title && orig_title != elem.title)
                    info.push(orig_title);
                if (elem.seasons_count)
                    info.push(Lampa.Lang.translate('online_mod_seasons_count') + ': ' + elem.seasons_count);
                if (elem.episodes_count)
                    info.push(Lampa.Lang.translate('online_mod_episodes_count') + ': ' + elem.episodes_count);
                elem.title = title;
            ...
            if (qualityMap) {
                ...
                if (idx !== -1) {
                    for (var i = idx; i < items.length; i++) {
                        var item = items[i];
                        if (qualityMap[item])
                            return qualityMap[item];
                    }

                    for (var _i = idx - 1; _i >= 0; _i--) {
                        var _item = items[_i];
                        if (qualityMap[_item])
                            return qualityMap[_item];
                    }
                }
            }
       ...
       */
        ...
        this.contextmenu = function(params) {
            ...
            params.item.on('hover:long', function() {
                ...
                if (Lampa.Helper)
                    Lampa.Helper.show('online_file', Lampa.Lang.translate('online_mod_file_helper'), params.item);
            });
        }
        ;
        /**
    ...
    function showStatus() {
        ...
        if (status.login) {
            if (status.is_pro)
                info = status.login + ' - PRO ' + Lampa.Lang.translate('filter_rating_to') + ' - ' + status.pro_date;
            else if (status.is_pro_plus)
                info = status.login + ' - PRO_PLUS ' + Lampa.Lang.translate('filter_rating_to') + ' - ' + status.pro_date;
            else
                info = status.login + ' - NO PRO';
        }
    ...
    function rezka2FillCookie(success, error) {
        ...
        network["native"](Utils.proxyLink(url, prox, prox_enc), function(json) {
            ...
            if (cookie) {
                ...
                if (cookie.indexOf('PHPSESSID=') == -1)
                    cookie = 'PHPSESSID=' + (sid || Utils.randomId(26)) + (cookie ? '; ' + cookie : '');
                var prox_enc2 = prox_enc;
                ...
                network["native"](Utils.proxyLink(host + '/', prox, prox_enc2), function(str) {
                    ...
                    if (verify_form) {
                        ...
                        try {
                            verify_cookie = (0,
                            eval)('"use strict"; (function(name, value){ return {name: name, value: value}; })' + verify_form[1] + ';');
                        } catch (e) {}
    ...
    function fancdnFillCookie(success, error) {
        ...
        network["native"](Utils.proxyLink(url, prox, prox_enc), function(str) {
            ...
            if (cookie) {
                ...
                if (cookie.indexOf('PHPSESSID=') == -1)
                    cookie = 'PHPSESSID=' + (sid || Utils.randomHex(32)) + (cookie ? '; ' + cookie : '');
                var prox_enc2 = prox_enc;
