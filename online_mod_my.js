...
function unifiedAppend(component, items, object, select_title, filter_items, choice, getStreamFunction, contextMenuFunction) {
    component.reset();
    var viewed = Lampa.Storage.cache('online_view', 5000, []);
    var last_episode = component.getLastEpisode(items);
    
    // Если включен случайный порядок, перемешиваем элементы
    if (Lampa.Storage.field('online_mod_random_order') === true) {
    ...
    items.forEach(function (element) {
        ...
        var hash_file = Lampa.Utils.hash(element.season ? 
            [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title, 
             filter_items.voice ? filter_items.voice[choice.voice] : ''].join('') : 
        ...
        item.on('hover:enter', function () {
            ...
            getStreamFunction(element, function (element) {
                element.loading = false;
                
                var playlist = buildPlaylist(items, element, select_title, getStreamFunction);
                var first = playlist.find(function(item) {
...
function getAllEpisodes(extract, filter_items, choice) {
    var allEpisodes = [];
    
    if (extract.seasons) {
        extract.seasons.forEach(function(season, seasonIndex) {
...
// Функция для построения плейлиста
function buildPlaylist(items, currentElement, select_title, getStreamFunction) {
    var playlist = [];
    
    // Если включен случайный порядок, используем перемешанный список
    if (Lampa.Storage.field('online_mod_random_order') === true) {
        var shuffledItems = shuffleArray(items);
        
        shuffledItems.forEach(function(elem) {
            if (elem.stream) {
                playlist.push({
                ...
                var cell = {
                    url: function(call) {
                        getStreamFunction(elem, function(elem) {
...
function buildPlaylist(items, currentElement, select_title, getStreamFunction) {
    ...
    function lumex(component, _object) {
      ...
      this.reset = function () {
        ...
        unifiedAppend(
          ...
          choice, 
          getStream, 
          component.contextmenu.bind(component)
        );
      ...
      function filtred() {
        // Если включен режим всех серий
        if (Lampa.Storage.field('online_mod_all_episodes') === true) {
            return getAllEpisodes(extract, filter_items, choice);
        }
    ...
    Lampa.Params.trigger('online_mod_random_order', true);
    ...
    template += "\n    <div class=\"settings-param selector\" data-name=\"online_mod_random_order\" data-type=\"toggle\">\n        <div class=\"settings-param__name\">#{online_mod_random_order}</div>\n        <div class=\"settings-param__value\"></div>\n    </div>";
